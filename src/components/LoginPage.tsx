'use client';

import React, { useState } from 'react';
import { useAuth } from '@/contexts/AuthContext';
import { useNotification } from '@/contexts/NotificationContext';
import { LogIn, Bell, Calendar, CheckSquare, Clock } from 'lucide-react';
import LoadingSpinner from './LoadingSpinner';
import FirebaseTest from './FirebaseTest';

const LoginPage: React.FC = () => {
  const { signInWithGoogle, loading } = useAuth();
  const { isSupported } = useNotification();
  const [isSigningIn, setIsSigningIn] = useState(false);

  const handleGoogleSignIn = async () => {
    try {
      setIsSigningIn(true);
      await signInWithGoogle();
    } catch (error) {
      console.error('Sign in error:', error);
      if (error instanceof Error && error.message.includes('Firebase not initialized')) {
        alert('Firebase not configured. Please set up your environment variables first.');
      } else {
        alert('Failed to sign in. Please try again.');
      }
    } finally {
      setIsSigningIn(false);
    }
  };

  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          <div className="mx-auto h-16 w-16 bg-blue-600 rounded-full flex items-center justify-center mb-4">
            <Clock className="h-8 w-8 text-white" />
          </div>
          <h2 className="text-3xl font-bold text-gray-900 mb-2">EveryTodo</h2>
          <p className="text-gray-600">Your persistent alarm and task manager</p>
        </div>

        <div className="bg-white rounded-lg shadow-lg p-8">
          <div className="space-y-6">
            <div className="text-center">
              <h3 className="text-xl font-semibold text-gray-900 mb-4">
                Sign in to continue
              </h3>
              <p className="text-sm text-gray-600 mb-6">
                Use your Google account to access your todos and alarms
              </p>
            </div>

            <button
              onClick={handleGoogleSignIn}
              disabled={isSigningIn || loading}
              className="w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isSigningIn || loading ? (
                <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-600"></div>
              ) : (
                <>
                  <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24">
                    <path
                      fill="#4285F4"
                      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                    />
                    <path
                      fill="#34A853"
                      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                    />
                    <path
                      fill="#FBBC05"
                      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                    />
                    <path
                      fill="#EA4335"
                      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                    />
                  </svg>
                  Continue with Google
                </>
              )}
            </button>

            {!isSupported && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4">
                <div className="flex">
                  <Bell className="h-5 w-5 text-yellow-400" />
                  <div className="ml-3">
                    <p className="text-sm text-yellow-800">
                      <strong>Notification permission required:</strong> Please allow notifications to receive alarm alerts.
                    </p>
                  </div>
                </div>
              </div>
            )}

                    <div className="bg-blue-50 border border-blue-200 rounded-md p-4">
                      <div className="flex">
                        <div className="ml-3">
                          <p className="text-sm text-blue-800">
                            <strong>Setup Required:</strong> To use this app, you need to configure Firebase. 
                            See the README.md file for setup instructions.
                          </p>
                        </div>
                      </div>
                    </div>

                    <FirebaseTest />
          </div>
        </div>

        <div className="text-center">
          <div className="grid grid-cols-3 gap-4 text-sm text-gray-600">
            <div className="flex flex-col items-center">
              <Bell className="h-6 w-6 mb-2 text-blue-600" />
              <span>Persistent Alarms</span>
            </div>
            <div className="flex flex-col items-center">
              <CheckSquare className="h-6 w-6 mb-2 text-green-600" />
              <span>Task Management</span>
            </div>
            <div className="flex flex-col items-center">
              <Calendar className="h-6 w-6 mb-2 text-purple-600" />
              <span>Calendar View</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default LoginPage;
